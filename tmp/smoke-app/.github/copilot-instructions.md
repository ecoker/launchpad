# smoke-app — Engineering Standards
...
def calculate_total(price, rate), do: price * (1 - rate)

# ❌ Hidden dependency — harder to test, surprising in production
def calculate_total(price), do: price * (1 - get_config_rate())
```
...
def format_status(status) do
  if status == :active do
    ...
    if status == :suspended, do: "Suspended", else: "Cancelled"
  end
end
```
...
defp maybe_filter_by_topic(query, topic), do: where(query, topic: ^topic)
```
